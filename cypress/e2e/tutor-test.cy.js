describe("My First Test", () => {
  it("Visits the Login Page and Enters Credentials", () => {
    ...
    //click filter button
    cy.get("button.ant-btn-primary.ant-btn-lg").contains("Filter").click();
    ...
    //Search box checking
    cy.get('input[placeholder="Search Team"]').clear().type("1");
    ...
    cy.get('li[data-value="COMP3900"]').click();
    cy.get('li[data-value="COMP9900"]').click();
    cy.get('input[placeholder="Search Unallocated Team"]').clear().type("1");
    cy.get('input[placeholder="Search Unallocated Team"]').should(
      "have.value",
      "1"
    );
    ...
    cy.get("button.ant-btn-primary.list-item-button")
      .contains("+ New Channel")
      .click();
    cy.get('input[placeholder="Search by name or email"]').clear().type("s");
    cy.get('input.ant-checkbox-input[value="2"]').click();

    //send message
    cy.get('input.ant-checkbox-input[value="2"]').should("be.checked");
    cy.contains("button", "Invite").click();
    cy.get("input#exampleFormControlInput2").clear().type("Test{enter}");

    //Give a name card
    cy.get(
      "button.MuiButtonBase-root.MuiIconButton-root.circle-buttonshare"
    ).click();
    ...
    //View Group Members
    cy.get(
      "button.MuiButtonBase-root.MuiIconButton-root.group-icon-button"
    ).click();
    cy.get(".ant-modal-wrap")
      .last()
      .find('svg[data-icon="close"]')
      .click({ force: true });
    ...
    //Switching channels
    cy.get("button.ant-btn-primary.list-item-button")
      .contains("span", "All Channel")
      .click();
    cy.contains(
      "li.ant-list-item",
      "Private Chat: tutor1 and student1"
    ).click();
    ...
    cy.get("button.ant-btn-primary")
      ...
      .click({ force: true });
    cy.contains("button.ant-btn-primary.ant-btn-dangerous", "Clear").click({
      force: true,
    });
  });
});
