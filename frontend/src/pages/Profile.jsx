...
const Profile = (props) => {
    ...
    if (editable) {
      const confirmUpdate = async () => {
        ...
        if (!bio || !field || !name || !organization || !skills || !avatar) {
          if (
            window.confirm(
              "Some fields are empty. Are you sure you want to update?"
            )
          ) {
            try {
              const response = await apiCall(
              ...
              if (response.msg) {
                ...
                setSnackbarContent(
                  response.error || "Failed to update user profile"
                );
                setAlertType("error");
              }
            } catch (error) {
  ...
  const handleChangePassword = async () => {
    ...
    if (!passwordRegex.test(newPassword)) {
      setSnackbarContent(
        "Password must be at least 8 characters long, including letter, number, and special character."
      );
      setAlertType("error");
      setAlertOpen(true);
      return;
          ...
          <Container className="p-4 wrapper" fluid>
            ...
            <Box sx={{ mt: 4 }}>
              ...
              {editable ? (
                ...
                <Typography
                  variant="body1"
                  sx={{ mt: 1, bgcolor: "grey.200", p: 2, borderRadius: 1 }}
                >
                  {bio}
                </Typography>
              )}
