...
import { StarFilled, PlusOutlined, FileTextOutlined } from "@ant-design/icons";
...
const ProjectProgress = (props) => {
  ...
  const loadUserData = async () => {
    ...
    const response = await apiCall(
      "GET",
      `v1/progress/get/detail/${item?.teamId}`,
      null,
      token,
      true
    );
    if (!response) {
      ...
      const allUserStories = response.sprints.reduce((acc, sprint) => {
        ...
        const userStoriesWithIndexes = sprint.userStoryList.map(
          (story, index) => ({
            ...story,
      ...
      setLoading(false);
      console.log("{grades: grades, comments: comments}", {
        grades: grades,
        comments: comments,
      });
    }
  ...
  const renderSprints = () => {
    return sprintData.map((sprint, index) => {
      const currentSprint = data?.sprints?.find(
        (s) => s.sprintNum === sprint.sprintNumber
      );
      ...
      return item ? (
        <CardBody className="">
          ...
          {/* list of user story */}
          <List
            dataSource={storys.filter(
              (story) => story.sprintNum === sprint.sprintNumber
            )}
            renderItem={(story, index) => (
              <List.Item className="list-item" key={story.userStoryId}>
                <List.Item.Meta
                  onClick={() =>
                    handleUserStoryClick(story, sprint.sprintNumber)
                  }
